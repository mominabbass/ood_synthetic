...
def setup_llama2_7b(model_name, params, all_train_sentences, all_train_labels, train_sentences, train_labels,
    ...
    if llamma2_7b_model is None:
        ...
        #               'model.layers.15': 1, 'model.layers.16': 1, 'model.layers.17': 1, 'model.layers.18': 1,
        #               'model.layers.19': 1, 'model.layers.20': 1, 'model.layers.21': 1, 'model.layers.22': 1,
        #               'model.layers.23': 1, 'model.layers.24': 1, 'model.layers.25': 1, 'model.layers.26': 1,
        #               'model.layers.27': 1, 'model.layers.28': 1, 'model.layers.29': 1, 'model.layers.30': 1,
        #               'model.layers.31': 1, 'model.norm': 1, 'lm_head': 1}
        ...
        # print('model.hf_device_map: ', llamma2_7b_model.hf_device_map)   #to print, set device_map='auto' in the previous line
...
def setup_llama2_13b(model_name, params, all_train_sentences, all_train_labels, train_sentences, train_labels, val_sentences, val_labels, test_sentences, test_labels):
    ...
    if llamma2_13b_model is None:
        if (params['train'] == False):
            ...
            if params['dataset'] == 'response_beavertails_unethical_OOD_discrimincation-hate':
                folder_name = "saved_checkpoints/synthetic_BT_discrimincation_hate_Llama-2-13b-hf_acc0.669.pt"
            elif params['dataset'] == 'response_beavertails_unethical_OOD_sexual-drug':
                folder_name = "saved_checkpoints/synthetic_BT_sexual_drug_Llama-2-13b-hf_acc0.872.pt"
            elif params['dataset'] == 'response_beavertails_unethical_OOD_mbpp':
                folder_name = "saved_checkpoints/synthetic_BT_mbpp_Llama-2-13b-hf_acc0.902.pt"
            elif params['dataset'] == 'response_beavertails_unethical_OOD_gsm8k':
                folder_name = "saved_checkpoints/synthetic_BT_gsm8k_Llama-2-13b-hf_acc0.899.pt"
            elif params['dataset'] == 'civil_comments_toxicity_OOD_toxigen':
                folder_name = "saved_checkpoints/synthetic_CC_toxigen_Llama-2-13b-hf_acc0.875.pt"
            elif params['dataset'] == 'civil_comments_toxicity_OOD_sst2':
                folder_name = "saved_checkpoints/synthetic_CC_sst2_Llama-2-13b-hf_acc0.897.pt"
            elif params['dataset'] == 'civil_comments_toxicity_OOD_mbpp':
                folder_name = "saved_checkpoints/synthetic_CC_mbpp_Llama-2-13b-hf_acc0.964.pt"
            else:
            ...
            for epoch in range(num_epochs):
                ...
                for chunk_id, val_chunk_prompts in enumerate(chunked_prompts):
                    with torch.no_grad():
                        _, resp = complete_llamma2_13b(val_chunk_prompts, params['label_dict'], normalize=True)
                ...
                checkpoint_name = f"synthetic_train_{dataset_name}_{model_name_save}_ep{epoch}_acc{val_acc}.pt".replace(
                    "/", "_"
                )
                # if (val_acc > prev_acc):
                # prev_acc = val_acc
            ...
            checkpoint_name = f"synthetic_train_{dataset_name}_{model_name_save}_last_acc{val_acc}.pt".replace(
                "/", "_"
            )
            llamma2_13b_model.save_pretrained("saved_checkpoints/{}".format(checkpoint_name))
